...
const List = () => {
  ...
  const CATEGORY =  'soccer' === pageNumber ? 
                        0 : 'baseball' === pageNumber ? 
                          1 :'esport' === pageNumber ? 
                            2 : ''
  ...
  useEffect(() => {
    axios.get(`http://localhost:5500/api/v1/post/category/${CATEGORY}`)
    .then((res) => {
    ...
    <div style={{padding: '0 162px'}}>
      ...
      <SectionTag margin="0 0 49px">
        ...
        <article style={noticeContent}>
          <Marquee
            style={{background: '#DFD6F9', height: '100%', margin: '0 0 0 20px'}}
            direction="left"
            speed={10}
